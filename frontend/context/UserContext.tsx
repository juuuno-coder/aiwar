...
export function UserProvider({ children }: { children: React.ReactNode }) {
    ...
    const refreshData = async () => {
        if (!user) return; // No user, no refresh needed (guest handled separately)

        setLoading(true);
        try {
            // [Strict Mode] Load inventory from DB only
            const inv = await loadInventory(user.uid);
            // Verify Inventory integrity
            const cleanInventory = inv.filter(c => c && c.id);
            setInventory(cleanInventory);

            // [Strict Mode] Load GameState (throws if DB fails)
            ...
            // Starter Pack Logic
            const hasReceived = state.hasReceivedStarterPack || (state as any).profile?.hasReceivedStarterPack;
            // Note: gameStorage returns simplified state. 'hasReceivedStarterPack' might be missing in interface but present in profile merge.
            // Actually, we should rely on PROFILE from useUserProfile, but gameStorage syncs from it.
            // Let's rely on the profile hook primarily for Starter Pack status if available.

            // Check Profile directly if possible, or trust state
            if (profile) {
                setStarterPackAvailable(!profile.hasReceivedStarterPack);
            }

            setError(null); // Clear any previous errors
        } catch (err) {
            console.error("CRITICAL: Failed to refresh user data", err);
            setError("데이터 동기화 오류: 서버로부터 계정 정보를 불러올 수 없습니다. (DB Connection Failed)");
        } finally {
            setLoading(false);
        }
    };
